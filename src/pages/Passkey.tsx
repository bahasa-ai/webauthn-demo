...
export default function () {
  ...
  const register = async () => {
    const { data: options } = await axios.post('/api/passkey/register', {}, {
      headers: { Authorization: `Bearer ${token?.access_token}` }
    })
    const resp: RegistrationCredentialJSON = await startRegistration(options)

    try {
      const { data } = await axios.post('/api/passkey/verifyRegister', resp, {
        headers: { Authorization: `Bearer ${token?.access_token}` }
      })
  ...
  const auth = async (useBrowserAutofill: boolean = true) => {
    const id = new Date().getTime().toString()
    const { data: options } = await axios.post('/api/passkey/auth', {
      attempt_id: id
    })
    const resp: AuthenticationCredentialJSON = await startAuthentication(options, useBrowserAutofill)

    try {
      const { data } = await axios.post('/api/passkey/verifyAuth', {
        ...resp,
        attempt_id: id
      })
      ...
      <Grid>
        <Col span={6}>
          <form onSubmit={form.onSubmit((data) => login(data))}>
            <Input.Wrapper>
              <Input placeholder="username" {...form.getInputProps('username')} autoComplete="webauthn" />
            </Input.Wrapper>
            <Input.Wrapper mt="md">
